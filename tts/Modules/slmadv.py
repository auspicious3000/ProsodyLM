...
from pdb_fairseq import set_trace
...
class SLMAdversarialLoss(torch.nn.Module):
    ...
    def forward(self, iters, y_rec_gt, y_rec_gt_pred, waves, mel_input_length, ref_text, ref_lengths, use_ind, s_trg, ref_s,
                prsinf, boundaries):
        text_mask = length_to_mask(ref_lengths).to(ref_text.device)
        bert_dur = self.model.bert(ref_text, attention_mask=(~text_mask).int())
        d_en = self.model.bert_encoder(bert_dur).transpose(-1, -2) 
        
        s_dur = torch.zeros_like(s_trg[:, 128:])
        s = s_trg[:, :128]
